...
res_decontprev_even_PR <- BM_decontam_prev_PR(
  otus_rel = otus_rel, metadata = metadata, Mock_info = Tax_class, 
  per_dilution = TRUE, dmax = 8) # spike: 1, dmock: 7
...
res_decontfreq_even_PR <- BM_decontam_freq_PR(
  otus_rel = otus_rel, metadata = metadata, Mock_info = Tax_class, 
  per_dilution = FALSE, dmax = 8) # spike: 1, dmock: 7
...
# Save all:
###save(list = ls()[grepl("res_.*_PR", ls())], file = paste0(file_directory, "Output/R_objects/1_BM_even_all_PR.RData"))
###load(paste0(file_directory, "Output/R_objects/1_BM_even_all_PR.RData"), verbose = TRUE)
# sourcetracker still missing
################################################################################
...
combined_results_PR %>% 
  ...
  filter(Dilution %in% c("D2", "D4", "D6", "D8")) %>% # only in Dmock
...
ggsave(paste0(file_directory, "Output/Plots/PR-curves_even.svg"),

...
res_decontprev_spike_PR <- BM_decontam_prev_PR(
  otus_rel = otus_rel, metadata = metadata, Mock_info = Tax_class, 
  per_dilution = TRUE, dmax = 1) # spike: 1, dmock: 7
...
# Save all:
###save(list = ls()[grepl("res_.*_PR", ls())], file = paste0(file_directory, "Output/R_objects/2_BM_spike_all_PR.RData"))
###load(paste0(file_directory, "Output/R_objects/2_BM_spike_all_PR.RData"), verbose = TRUE)
...
ggsave(paste0(file_directory, "Output/Plots/PR-curves_spike.svg"),
